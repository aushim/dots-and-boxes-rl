...
class raw_env(AECEnv, EzPickle):
    ...
    def __init__(self, render_mode: str | None = None, screen_scaling: int = 9, rows: int = 5, columns: int = 5, num_agents: int = 2):
        ...
        self._columns = columns  if columns <=10 else 10 # The number of columns in the grid
        self._num_agents = num_agents if num_agents <=6 else 6  # The number of agents in the environment
        ...
        self.action_spaces = {i: spaces.Discrete(self._num_edges) for i in self.agents}
    ...
    def observe(self, agent):
        observation = self._grid._grid
        legal_moves = self._legal_moves() if agent == self.agent_selection else []

        action_mask = np.zeros(self._num_edges, "int8")
    ...
    def step(self, action):
        ...
        action_valid, first_cell, second_cell, first_cell_closed_after_action, second_cell_closed_after_action = self._grid._apply_action(action)
        ...
        if action_valid:
            ...
            if first_cell_closed_after_action:
                self._cell_belongs_to[first_cell[0]][first_cell[1]] = self.agent_selection
            if second_cell_closed_after_action:
                self._cell_belongs_to[second_cell[0]][second_cell[1]] = self.agent_selection
            ...
            # TODO: Remove this else block because flow will never reach here
            reward = -1
            next_agent = self.agent_selection
        ...
        self.terminations={i: self._grid._check_if_all_edges_closed() for i in self.agents}
    ...
    def reset(self, seed=None, options=None):
        ...
        self._last_reward = 0
        self._cell_belongs_to = [['' for _ in range(self._columns)] for _ in range(self._rows)]
    ...
    def render(self):
        if self.window is None and self.render_mode == "human":
            pygame.init()
            pygame.display.init()
            self.window = pygame.display.set_mode((self._window_size, self._window_size))
                ...
                for y in range(self._columns):
                    if self._grid._check_edge_already_closed([x, y], "bottom"):
                        pygame.draw.line(
                            canvas,
                            0,
                            (x_offset + pix_square_size * y, y_offset + (pix_square_size * self._rows)),
                            (x_offset + pix_square_size * (y + 1), y_offset + (pix_square_size * self._rows)),
                            width=3,
                        )
                ...
                if y == self._columns - 1:
                    if self._grid._check_edge_already_closed([x, y], "right"):
                        pygame.draw.line(
                            canvas,
                            0,
                            (x_offset + pix_square_size * self._columns, y_offset + pix_square_size * x),
                            (x_offset + pix_square_size * self._columns, y_offset + pix_square_size * (x + 1)),
                            width=3,
                        )
        ...
        if self._last_action is not None: 
            ...
            font = pygame.font.SysFont("Arial", 30)
            text = font.render(
                "Player: " + str(self._last_player if self._last_player is not None else None),
                True,
            ...
            action_cells = self._grid._get_cells_from_action(self._last_action) if self._last_action is not None else None
            text = font.render(
            ...
            canvas.blit(
                ...
                (
                    x_offset + pix_square_size * self._columns / 2 - text.get_width() / 2,
                    y_offset + pix_square_size * (self._rows + 1) + text.get_height(),
                ),
            )
            text = font.render(
            ...
            canvas.blit(
                ...
                (
                    x_offset + pix_square_size * self._columns / 2 - text.get_width() / 2,
                    y_offset + pix_square_size * (self._rows + 1) + 2 * text.get_height(),
                ),
            )
        ...
        font = pygame.font.SysFont("Arial", 30)
        text = font.render(
            "Next Turn: Player " + str(self.agent_selection),
            True,
        ...
        for i in range(self._num_agents):
            text = font.render(
                "Player " + str(chr(i + 65)) + ": " + str(self._cumulative_rewards[str(chr(i + 65))]),
                True,
            ...
            canvas.blit(
                ...
                (
                    x_offset + pix_square_size * self._columns / 2 - text.get_width() / 2,
                    y_offset - ((text.get_height() * 1.1) * (self._num_agents - i + 1)),
                ),
            )
